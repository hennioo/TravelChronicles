// Final Fix mit HEIC-Unterstützung
  ...
  res.send(`
    ...
    <body>
      ...
      <div class="loading-indicator" id="loadingIndicator">
        ...
        <div id="loadingText">Bild wird verarbeitet</div>
      </div>
            ...
            <form id="locationForm">
              ...
              <div class="form-group">
                ...
                <small style="color: #999; display: block; margin-top: 5px;">
                  Unterstützte Formate: JPG, PNG, HEIC. Max. 20MB.<br>
                  HEIC-Bilder werden automatisch konvertiert (kann etwas Zeit dauern).
                </small>
              </div>
...
async function convertHeicToJpeg(buffer) {
  ...
  try {
    ...
    const jpegBuffer = await heicConvert({
      ...
      quality: 0.9        // Qualität (0-1)
    });
    
    log('HEIC zu JPEG Konvertierung erfolgreich');
    return jpegBuffer;
  } catch (error) {
...
app.post('/api/locations', requireAuth, upload.single('image'), async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      // Überprüfe, ob es sich um ein HEIC/HEIF-Bild handelt
      let processedImageData = req.file.buffer;
      let processedImageType = req.file.mimetype;
      
      const fileName = req.file.originalname.toLowerCase();
      const isHeicFile = isHeic === 'true' || fileName.endsWith('.heic') || fileName.endsWith('.heif') || req.file.mimetype === 'image/heic' || req.file.mimetype === 'image/heif';
      
      if (isHeicFile) {
        try {
          log('HEIC-Bild erkannt, starte Konvertierung');
          processedImageData = await convertHeicToJpeg(req.file.buffer);
          processedImageType = 'image/jpeg';
          log('HEIC-Konvertierung abgeschlossen');
        } catch (error) {
          log('Fehler bei der HEIC-Konvertierung:', error);
          return res.status(400).json({ 
            error: 'Fehler bei der Verarbeitung des HEIC-Bildes. Bitte lade ein JPG- oder PNG-Bild hoch.' 
          });
        }
      }
