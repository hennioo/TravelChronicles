// Dieses Script erstellt eine standalone Lösung für das Bildproblem
const fs = require('fs');
...
console.log('Schreibe HTML-Templates...');
try {
  fs.writeFileSync(path.join(__dirname, 'dist', 'map.html'), mapHtml);
  fs.writeFileSync(path.join(__dirname, 'dist', 'login.html'), loginHtml);
  console.log('HTML-Templates erfolgreich geschrieben');
} catch (error) {
  console.error('Fehler beim Schreiben der HTML-Templates:', error);
}

// Schreibe Server-Code
console.log('Füge Base64-Endpunkt zum Server hinzu...');
try {
  ...
  console.error('Fehler beim Aktualisieren des Server-Codes:', error);
}
