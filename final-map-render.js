// Finale Version für Render mit allen Funktionen
...
// Eingebettetes Hintergrundbild (Base64)
const backgroundImage = `
<svg xmlns="http://www.w3.org/2000/svg" width="800" height="600" viewBox="0 0 800 600">
  <rect width="800" height="600" fill="#1a1a1a"/>
  <text x="400" y="300" font-family="Arial" font-size="40" text-anchor="middle" fill="#f59a0c">Susibert</text>
  <text x="400" y="350" font-family="Arial" font-size="20" text-anchor="middle" fill="#f5f5f5">Reise mit Liebe</text>
</svg>
`;
  ...
  if (process.env.DATABASE_URL) {
    ...
    pool.query('SELECT NOW() as now')
      ...
      .then(function(tabResult) {
        ...
        // Stelle sicher, dass alle Felder existieren (migrations-ersatz)
...
// Stellt sicher, dass alle benötigten Felder in der Datenbank vorhanden sind
function ensureDbSchema() {
  // Tabelle & Felder prüfen und ggf. anlegen
  const queries = [
    // Locations-Tabelle und Felder
    "CREATE TABLE IF NOT EXISTS locations (id SERIAL PRIMARY KEY, name TEXT, date DATE, description TEXT, highlight TEXT, latitude TEXT, longitude TEXT, country_code TEXT, image TEXT)",
    // Überprüfe, ob die Spalte image existiert und füge sie hinzu, falls nicht
    "DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='image') THEN ALTER TABLE locations ADD COLUMN image TEXT; END IF; END $$"
  ];
  
  // Führe alle Queries sequentiell aus
  return queries.reduce((p, query) => 
    p.then(() => pool.query(query).catch(err => console.log('Migration-Query-Fehler (kann normal sein):', err.message)))
  , Promise.resolve());
}
...
app.get('/', function(req, res) {
  ...
  const coupleImageBase64 = `
    data:image/svg+xml;base64,${Buffer.from(`
      <svg xmlns="http://www.w3.org/2000/svg" width="500" height="500" viewBox="0 0 500 500">
        <rect width="500" height="500" fill="#1a532a"/>
        <text x="250" y="250" font-family="Arial" font-size="30" text-anchor="middle" fill="#f5f5f5">Susibert</text>
        <text x="250" y="290" font-family="Arial" font-size="20" text-anchor="middle" fill="#f5f5f5">Unsere Reisekarte</text>
      </svg>
    `).toString('base64')}
  `;
...
<head>
  ...
  <style>
    body {
      ...
      align-items: center;
      height: 100vh;
      background-image: url("${coupleImageBase64}");
    ...
    <body>
      ...
      <div class="add-location-form" id="addLocationForm">
        ...
        <div class="form-content">
          <div id="formMessage" style="margin-bottom: 15px; display: none; padding: 10px; border-radius: 4px;"></div>
      ...
      <script>
        ...
        function showLocationDetail(location) {
          ...
          if (location.date) {
            const date = new Date(location.date).toLocaleDateString();
            content += \`<div class="location-meta">Datum: \${date}</div>\`;
          }
          ...
          content += \`<div class="location-meta">Koordinaten: \${location.latitude}, \${location.longitude}</div>\`;
        ...
        function renderLocationList(locations) {
          ...
          let html = '';
          locations.forEach(location => {
            const date = location.date ? new Date(location.date).toLocaleDateString() : '';
        ...
        function saveLocation(formData) {
          ...
          formMessage.style.backgroundColor = '#333';
          formMessage.style.color = 'white';
          formMessage.textContent = 'Speichere Ort...';
          ...
          .then(data => {
            ...
            formMessage.style.color = '#00aa00';
            formMessage.textContent = 'Ort erfolgreich gespeichert!';
          ...
          .catch(error => {
            ...
            formMessage.style.color = '#aa0000';
            formMessage.textContent = 'Fehler beim Speichern: ' + error.message;
