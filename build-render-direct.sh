...
# Aggressiver Build-Script für Render mit direkter Kontrolle über die Datenbankstruktur
set -ex

echo "=== Build-Script mit direkter Datenbanksteuerung ==="
...
# 3. Direkte Datenbank-Einrichtung
echo "Führe direkte Datenbank-Einrichtung aus..."
node direct-setup.js || echo "Datenbank-Setup hat Fehler zurückgegeben, aber wir machen trotzdem weiter"

# 4. Server-Code erstellen (alles in einer Datei)
echo "Erstelle vereinfachten Server-Code..."
cat > dist/index.js << 'EOF'
// Einfacher Server für Render mit fester Tabellenstruktur
...
async function connectToDatabase() {
  try {
    ...
    if (!tableExists) {
      console.log('Tabelle muss erstellt werden - Achtung, dies sollte bereits im Setup erfolgt sein!');
      await createTables();
...
async function createTables() {
  try {
    // HINWEIS: Diese Tabelle sollte bereits im Setup-Script erstellt worden sein
    await pool.query(`
...
app.get('/admin', requireAuth, function(req, res) {
  ...
  const adminHtml = '<!DOCTYPE html>\n' +
    ...
    '          \n' +
    '          if (data.success) {\n' +
    '            resultElement.textContent = "Datenbank erfolgreich zurückgesetzt. Die Seite wird neu geladen...";\n' +
    '            resultElement.className = "action-result success";\n' +
    '            \n' +
...
app.post('/api/admin/reset-database', requireAuth, async (req, res) => {
  ...
  try {
    // Tabelle komplett löschen und neu erstellen
