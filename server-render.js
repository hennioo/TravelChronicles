...
app.delete('/api/locations/:id', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const findResult = await pool.query('SELECT * FROM locations WHERE id = $1', [id]);
    
    if (findResult.rows.length === 0) {
      return res.status(404).json({ error: 'Standort nicht gefunden' });
    }
    
    const location = findResult.rows[0];
    
    // Lösche den Ort aus der Datenbank
    const deleteResult = await pool.query('DELETE FROM locations WHERE id = $1 RETURNING *', [id]);
    ...
    res.json({ success: true, message: 'Standort erfolgreich gelöscht', location });
  } catch (error) {
            ...
            try {
              ...
              if (map) {
                ...
                if (!isNaN(lat) && !isNaN(lng)) {
                  ...
                  for (var i = 0; i < 5; i++) {
                    var radius = 50000 * (1 - i/5);
                    var opacity = 0.05 + (i / 5) * 0.3;
