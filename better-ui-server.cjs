// Verbesserter Server für Susibert mit UI-Optimierungen
...
// Login-Seite mit verbessertem Design
...
app.get('/fix-database', requireAuth, async (req, res) => {
  try {
    ...
    // Für andere Spalten, die möglicherweise Probleme verursachen
    try {
      ...
      console.log('Einige Spalten konnten nicht angepasst werden');
    }
...
app.post('/api/locations', upload.single('image'), async (req, res) => {
  try {
    ...
    try {
      // Zuerst mit 'title' versuchen (für neue Datenbank)
      const result = await pool.query(
      ...
      try {
        // Dann mit 'name' versuchen (für alte Datenbank)
        const result = await pool.query(
