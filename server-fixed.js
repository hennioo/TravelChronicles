  ...
  pool.query('SELECT * FROM locations ORDER BY id DESC')
    .then(function(result) {
      ...
      for (var i = 0; i < result.rows.length; i++) {
        ...
        // Konstruiere eine absolute URL zum Bild für Frontend-Flexibilität
        var fullImagePath = 'https://susio.site/uploads/' + originalFilename;
        
        // Problem mit doppelter Domain beheben (falls es auftritt)
        if (fullImagePath.includes('http://susio.sitehttps://')) {
          fullImagePath = fullImagePath.replace('http://susio.sitehttps://', 'https://');
        }
...
// Neue Kartenansicht mit Sidebar importieren
const { generateMapHtml } = require('./map-layout');
