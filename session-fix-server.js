// Verbesserte Version mit permanentem Fix für Session-Probleme
...
console.log('Session-Fix aktiviert: Ja');
...
app.get('/', function(req, res) {
  // Neue Session erstellen und in Cookie speichern
...
app.post('/login', (req, res) => {
  ...
  // Session-ID aus Cookie oder aus Request-Body lesen
  ...
  if (accessCode === ACCESS_CODE) {
    ...
    // Session-ID in Cookie aktualisieren (24 Stunden)
    res.cookie('sessionId', sessionId, { 
      maxAge: 24 * 60 * 60 * 1000, 
      httpOnly: true,
      secure: process.env.NODE_ENV === 'production'
...
function getSessionId(req) {
  // Session-ID aus verschiedenen Quellen probieren
  return req.cookies.sessionId || req.query.sessionId || req.body.sessionId;
}
...
function requireAuth(req, res, next) {
  ...
  if (!sessionId || !sessions[sessionId] || !sessions[sessionId].authenticated) {
    console.log('Nicht authentifiziert, leite zur Startseite um');
    return res.redirect('/');
  }
  
  // Session-ID für die weitere Verwendung verfügbar machen
        ...
        function handleFormSubmit(e) {
          ...
          .then(response => {
              ...
              return response.text().then(text => {
                try {
                  ...
                  if (e instanceof SyntaxError) {
                    debug('Fehler beim Parsen der Antwort (nicht ok):', text);
                    
                    if (text.includes('<title>Susibert</title>') || text.includes('login')) {
                      // Session abgelaufen, zur Login-Seite umleiten
            ...
            if (contentType && contentType.indexOf('application/json') === -1) {
              return response.text().then(html => {
                debug('Fehler beim Parsen der Antwort (ok):', html);
          ...
          .catch(error => {
            ...
            if (error.message.includes('Session abgelaufen')) {
              // Nichts tun, wir leiten bereits um
            } else {
      ...
      <body>
        ...
        <div class="container">
          ...
          <div class="card">
            ...
            <p>Session-Fix aktiviert: <strong>Ja</strong></p>
