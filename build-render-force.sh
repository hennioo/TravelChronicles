...
# Alles-in-einem Build-Script für Render mit erzwungener Tabellenerstellung
set -ex

echo "=== Build-Script mit aggressiver DB-Korrektur ==="
...
// STARK AGGRESSIVES RESET UND SETUP DER DATENBANK
// Diese Funktion löscht die Tabelle und erstellt sie neu, wenn nötig
async function forceCreateTables() {
  try {
    console.log('FORCE: Versuche die locations-Tabelle zu löschen und neu zu erstellen...');
    
    // Versuche vorhandene Tabelle zu löschen
    try {
      await pool.query('DROP TABLE IF EXISTS locations CASCADE');
      console.log('FORCE: Tabelle wurde gelöscht (falls sie existierte)');
    } catch (dropError) {
      console.error('FORCE: Fehler beim Löschen der Tabelle:', dropError);
      // Weitermachen, auch wenn Fehler auftritt
    }
    
    // Tabelle neu erstellen mit allen benötigten Spalten
    try {
      await pool.query(`
      ...
      `);
      console.log('FORCE: Tabelle wurde neu erstellt');
      return true;
    } catch (createError) {
      console.error('FORCE: Fehler beim Erstellen der Tabelle:', createError);
      return false;
    }
  } catch (error) {
    console.error('FORCE: Allgemeiner Fehler bei Tabellenerstellung:', error);
    return false;
  }
}
...
async function connectToDatabase() {
  try {
    ...
    // WICHTIG: Hier führen wir IMMER die aggressive Tabellenerstellung durch
    // Dies ist eine wichtige Änderung gegenüber den vorherigen Versionen
    const tableCreated = await forceCreateTables();
    if (tableCreated) {
      console.log('Tabellen wurden erfolgreich neu erstellt');
    } else {
      console.error('Tabellen konnten nicht erstellt werden');
    }
    
    // Demonstration-Ort hinzufügen, wenn Tabelle leer ist
    try {
      const countResult = await pool.query('SELECT COUNT(*) FROM locations');
      if (countResult.rows[0].count === '0') {
        console.log('Tabelle ist leer, füge Demo-Ort hinzu...');
        await pool.query(`
          INSERT INTO locations (title, latitude, longitude, description) 
          VALUES ('Demo-Ort', 51.5074, -0.1278, 'Ein Demo-Ort zum Testen')
        `);
        console.log('Demo-Ort wurde hinzugefügt');
      }
    } catch (error) {
      console.error('Fehler beim Hinzufügen des Demo-Orts:', error);
    }
...
app.get('/admin', requireAuth, function(req, res) {
  ...
  const adminHtml = '<!DOCTYPE html>\n' +
    ...
    '      <h2 class="section-title">Datenbank reparieren und zurücksetzen</h2>\n' +
    '      <div class="warning">\n' +
    ...
    '      <button id="checkDbBtn" class="admin-button green">Tabellen-Status prüfen</button>\n' +
...
app.post('/api/admin/reset-database', requireAuth, async (req, res) => {
  ...
  try {
    // Aggressive Neuinitialisierung der Tabellen
    const success = await forceCreateTables();
    
    if (!success) {
      return res.status(500).json({ error: 'Fehler beim Zurücksetzen der Datenbank' });
    }
