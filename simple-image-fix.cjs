// Einfache Version mit direkter Bildverarbeitung ohne HEIC-Konvertierung
...
console.log('Starte Server mit direkter Bildverarbeitung...');
console.log('Datenbank-URL vorhanden:', !!DATABASE_URL);
...
try {
  pool = new Pool({
    ...
    max: 5 // Reduzierte Anzahl der Verbindungen
  });
            ...
            <form id="locationForm">
              ...
              <div class="form-group">
                ...
                <small style="color: #999; display: block; margin-top: 5px;">
                  Unterstützte Formate: JPG, PNG. Max. 20MB.<br>
                  HEIC-Bilder vom iPhone bitte vorher in der Fotos-App in JPG umwandeln.
                </small>
              </div>
...
app.get('/api/locations', requireAuth, async (req, res) => {
  try {
    ...
    try {
      ...
      console.log('Fehler beim Abrufen der Orte mit title-Spalte:', error.message);
      try {
        console.log('Versuche erneut mit name-Spalte');
        result = await pool.query('SELECT id, name AS title, latitude, longitude, description FROM locations ORDER BY id DESC');
        console.log(`${result.rows.length} Orte abgerufen (mit name-Spalte)`);
      } catch (error2) {
        console.log('Fehler beim Abrufen der Orte mit name-Spalte:', error2.message);
    ...
    console.log('Fehler beim Abrufen der Orte:', error.message);
    res.status(500).json([]);
  }
});
...
app.post('/api/locations', requireAuth, upload.single('image'), async (req, res) => {
  try {
    console.log('Starte Hinzufügen eines neuen Ortes');
    console.log('Request body:', req.body);
    console.log('File:', req.file ? `${req.file.originalname}, ${req.file.size} Bytes` : 'Kein Bild');
    ...
    try {
      console.log('Erstelle Thumbnail');
      thumbnailData = await sharp(req.file.buffer)
        .resize(60, 60, { fit: 'cover' })
        .toBuffer();
      console.log('Thumbnail erstellt, Größe:', thumbnailData.length, 'Bytes');
    } catch (error) {
      console.error('Fehler beim Erstellen des Thumbnails:', error.message);
      // Thumbnail-Fehler ignorieren, weitermachen
    }
    
    // In die Datenbank einfügen
    try {
      console.log('Starte DB-Einfügung...');
      
      // Eine Verbindung aus dem Pool holen
      const client = await pool.connect();
      console.log('DB-Verbindung erhalten');
      ...
      try {
        ...
        console.error('Fehler bei DB-Einfügung:', error.message);
        throw error;
      }
    } catch (error) {
