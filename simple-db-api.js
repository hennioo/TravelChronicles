// Einfache Version mit optimierter Bildverarbeitung
  ...
  res.send(`
    ...
    <body>
          ...
          <div class="form-container" id="addLocationForm">
            ...
            <form id="locationForm">
              ...
              <div class="form-group">
                ...
                <input type="file" id="locationImage" class="form-input" accept="image/jpeg,image/png" required>
                <small style="color: #999; display: block; margin-top: 5px;">
                  Nur JPG und PNG unterstützt. Max. 2MB.<br>
                  <strong>Wichtig:</strong> HEIC-Bilder vom iPhone bitte vorher in JPG umwandeln.
                </small>
              </div>
      ...
      <script>
        ...
        function showLoading(text = "Bild wird verarbeitet...") {
        ...
        function validateImageFile(file) {
          // Maximale Dateigröße prüfen (2 MB)
          const maxSizeBytes = 2 * 1024 * 1024; // 2 MB
          if (file.size > maxSizeBytes) {
            return {
              valid: false,
              message: 'Das Bild ist zu groß (max. 2 MB erlaubt)'
            };
          }
        ...
        function handleFormSubmit(e) {
          ...
          showLoading('Bild wird verarbeitet und hochgeladen...');
          
          // Datei im Browser verkleinern, um Datenübertragungsprobleme zu minimieren
          compressImageIfNeeded(file)
            .then(processedFile => {
              ...
              debug('Sende verkleinerte Daten an /api/locations (Bildgröße: ' + 
                   processedFile.size + ' Bytes)');
            ...
            .then(response => {
              ...
              if (!response.ok) {
                debug('Response nicht OK, Status:', response.status);
                return response.text().then(text => {
                  ...
                  try {
                    ...
                    errorMessage = 'Unerwartete Antwort vom Server: ' + text;
                  }
        ...
        async function compressImageIfNeeded(file) {
          // Wenn die Datei klein genug ist, keine Kompression notwendig
          if (file.size < 500 * 1024) { // Weniger als 500KB
            return file;
          }
          
          debug('Bild wird komprimiert...', {
            originalSize: file.size, 
            originalType: file.type,
            name: file.name
          });
          
          showLoading('Bild wird komprimiert...');
          
          try {
            // Konvertiere die Datei in ein Bild-Element
            ...
            // Berechne neue Dimensionen (max 1280px Breite/Höhe)
            let width = img.width;
            ...
            // Konvertiere das Canvas zu einem Blob mit reduzierter Qualität
            const blob = await new Promise(resolve => {
              canvas.toBlob(resolve, 'image/jpeg', 0.8);
            });
            
            // Erstelle ein neues File-Objekt
            const compressedFile = new File([blob], file.name.replace(/\.\w+$/, '.jpg'), {
              type: 'image/jpeg',
              lastModified: new Date().getTime()
            });
            
            debug('Bild komprimiert', {
              originalSize: file.size,
            ...
            debug('Fehler bei der Bildkomprimierung:', error);
            // Bei Fehler das Original zurückgeben
            return file;
          }
        }
...
const upload = multer({
  storage: storage,
  limits: { fileSize: 2 * 1024 * 1024 } // 2MB Limit für bessere Performance
});
...
app.post('/api/locations', requireAuth, upload.single('image'), async (req, res) => {
  ...
  try {
    ...
    // Hier optimieren wir das Bild mit Sharp für die Datenbank
    let imageBuffer = req.file.buffer;
    let imageType = req.file.mimetype;
    
    try {
      // Bild mit Sharp optimieren
      ...
      console.log('Bild optimiert von', req.file.size, 'auf', imageBuffer.length, 'Bytes');
    } catch (error) {
      console.error('Fehler bei der Bildoptimierung:', error.message);
      // Bei Fehler weitermachen mit dem Originalimage
    }
    ...
    try {
      ...
      // Füge erst die Metadaten ein, dann das Bild in einem separaten UPDATE
      try {
        console.log('Füge Metadaten ein...');
        const metadataQuery = 'INSERT INTO locations (title, latitude, longitude, description) VALUES ($1, $2, $3, $4) RETURNING id';
        const metadataParams = [title, latitude, longitude, description || ''];
        
        const result = await client.query(metadataQuery, metadataParams);
        const insertedId = result.rows[0].id;
        console.log('Metadaten erfolgreich eingefügt, ID:', insertedId);
        
        // Jetzt das Bild mit einem separaten Update
        console.log('Füge Bild hinzu...');
        const imageQuery = 'UPDATE locations SET image_data = $1, image_type = $2 WHERE id = $3';
        await client.query(imageQuery, [imageBuffer, imageType, insertedId]);
        console.log('Bild erfolgreich hinzugefügt');
