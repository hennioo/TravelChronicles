// Susibert App - Vollständige Anwendung
// CommonJS Format (.cjs) für Render-Kompatibilität
...
// Zugangscode (aus Umgebungsvariable oder Standard)
const ACCESS_CODE = process.env.ACCESS_CODE || 'suuuu';

// Temporärer Uploads-Ordner erstellen, falls er nicht existiert
...
async function compressImage(buffer, mimeType) {
  ...
  try {
    // Wenn es ein HEIC/HEIF Format ist, müssen wir es zuerst konvertieren
    if (mimeType.includes('heic') || mimeType.includes('heif')) {
      console.log('HEIC/HEIF Bild erkannt, konvertiere zu JPEG');
      
      // Da heic-convert ein ES Module ist und wir in CommonJS sind,
      // nutzen wir hier direkt Sharp für die Konvertierung
...
app.get('/couple-image', async (req, res) => {
  try {
    ...
    // Fallback: Standardbild senden, falls vorhanden
    const defaultImagePath = path.join(__dirname, 'public', 'default-couple.jpg');
    if (fs.existsSync(defaultImagePath)) {
      return res.sendFile(defaultImagePath);
    }
    
    // Zweiter Fallback: Ein anderes Bild aus der Datenbank
    const altResult = await pool.query('SELECT * FROM locations WHERE image IS NOT NULL LIMIT 1');
    if (altResult.rows.length > 0 && altResult.rows[0].image) {
      const imageBase64 = altResult.rows[0].image;
      const imageType = altResult.rows[0].image_type || 'image/jpeg';
    ...
    // Kein Bild gefunden
    res.status(404).send('Kein Bild verfügbar');
  } catch (error) {
      ...
      <div class="form-container" id="location-form-container">
        <div class="form-box">
          ...
          <form id="location-form">
            ...
            <input type="hidden" id="form-location-id">
      ...
      <script>
        ...
        function addLocationToSidebar(location) {
          ...
          locationItem.innerHTML = \`
            ...
            <img src="/api/locations/\${location.id}/image?sessionId=\${sessionId}" 
                 alt="\${location.title}" 
        ...
        async function showLocationDetail(id) {
          try {
            ...
            document.getElementById('detail-image').src = \`/api/locations/\${id}/image?sessionId=\${sessionId}&t=\${Date.now()}\`;
        ...
        document.addEventListener('DOMContentLoaded', function() {
          ...
          document.getElementById('location-form').addEventListener('submit', async function(e) {
            ...
            // Wenn ein Bild ausgewählt wurde, hinzufügen
            if (imageFile) {
    ...
    <body>
      <div class="container">
        ...
        <div class="section">
          ...
          <form id="couple-image-form">
            <div class="form-group">
              <label for="couple-image">Neues Pärchenbild</label>
              <input type="file" id="couple-image" accept="image/*" required>
            </div>
      ...
      <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
          ...
          document.getElementById('couple-image-form').addEventListener('submit', async function(e) {
            ...
            const imageFile = document.getElementById('couple-image').files[0];
            if (!imageFile) return;
...
app.get('/api/locations/:id/image', requireAuth, async (req, res) => {
  try {
    ...
    // Thumbnail oder Originalbild laden
    const query = thumbnail 
      ? 'SELECT thumbnail, image_type FROM locations WHERE id = $1'
      : 'SELECT image, image_type FROM locations WHERE id = $1';
    ...
    const imageBase64 = thumbnail ? result.rows[0].thumbnail : result.rows[0].image;
    const imageType = result.rows[0].image_type || 'image/jpeg';
    ...
    // Cache-Header für Bilder (außer bei explizitem Cache-Busting)
    if (!req.query.t) {
      res.set('Cache-Control', 'public, max-age=3600'); // 1 Stunde cachen
    } else {
      // Bei Cache-Busting (Timestamp in der URL) kein Caching
