// Ultra-einfache Version mit maximaler Fehlerbehandlung
...
app.post('/api/locations', requireAuth, upload.single('image'), async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // Minimale Daten (ohne Bilder) einfügen
      try {
        console.log('Teste minimale Einfügung ohne Bild...');
        const testQuery = 'INSERT INTO locations (title, latitude, longitude, description) VALUES ($1, $2, $3, $4) RETURNING id';
        const testParams = [title, latitude, longitude, description || ''];
        
        console.log('Führe minimale Einfügung aus...');
        const result = await client.query(testQuery, testParams);
        const insertedId = result.rows[0].id;
        console.log('Minimale Einfügung erfolgreich, ID:', insertedId);
        
        // Jetzt das Bild separat einfügen
        console.log('Füge Bild hinzu...');
        const updateQuery = 'UPDATE locations SET image_data = $1, image_type = $2 WHERE id = $3';
        await client.query(updateQuery, [req.file.buffer, req.file.mimetype, insertedId]);
        ...
        await client.query('ROLLBACK');
        console.error('Einfügung fehlgeschlagen:', error.message);
        
        // Fehler zurückgeben
        res.status(500).json({ error: 'Datenbank-Fehler: ' + error.message });
      }
    } finally {
      client.release();
    ...
    console.error('Allgemeiner Fehler bei /api/locations:', error.message);
