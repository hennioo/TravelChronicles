// Server mit eingebautem JavaScript-Fix für das Upload-Problem
...
console.log('Eingebauter Upload-Fix: AKTIV');
...
// Kartenansicht mit Leaflet.js und eingebautem JavaScript-Fix
  ...
  res.send(`
    ...
    <body>
      ...
      <script>
        ...
        // Upload-Handler wird direkt hier überschrieben
        fixUploadHandler();
        ...
        // Upload-Handler überschreiben
        function fixUploadHandler() {
          debug('Überschreibe Upload-Handler');
          
          locationForm.addEventListener('submit', function(e) {
            e.preventDefault();
            debug('Upload-Form-Handler aufgerufen');
            ...
            if (file.size > ${MAX_FILE_SIZE}) {
              showError(\`Das Bild ist zu groß: \${(file.size / (1024 * 1024)).toFixed(2)} MB. Maximal erlaubt sind ${MAX_FILE_SIZE / (1024 * 1024)} MB.\`);
              return;
            }
            
            debug('Formularwerte:', { title, lat, lng, desc, fileName: file.name, fileSize: file.size });
            ...
            debug('Sende Anfrage mit Session-ID in URL und FormData');
            ...
            .then(response => {
              debug('Antwort Status:', response.status);
              
              const contentType = response.headers.get('content-type');
              if (contentType && contentType.indexOf('application/json') === -1) {
                return response.text().then(html => {
                  debug('Fehler! Keine JSON-Antwort:', html.substring(0, 100));
          ...
          debug('Upload-Handler erfolgreich überschrieben');
        }
      ...
      <body>
        ...
        <div class="container">
          ...
          <div class="card">
            ...
            <p>Hochladen-Fix: <strong>AKTIV</strong></p>
